...
const uint32_t initial_h[5] = {0x67452301u, 0xEFCDAB89u, 0x98BADCFEu,
                               0x10325476u, 0xC3D2E1F0u};

/* Ordering of message words.  Based on the permutations rho(i) and pi(i),
 * defined as follows:
 *
 *  rho(i) := { 7, 4, 13, 1, 10, 6, 15, 3, 12, 0, 9, 5, 2, 14, 11, 8 }[i]  0 <=
 * i <= 15
 *
 *  pi(i) := 9*i + 5 (mod 16)
 *
...
void ripemd160_compress(ripemd160_state* self) {
  ...
  for (w = 0; w < 16; w++) { /* left line */
    T = ROL(SL[round][w],
            AL + F1(BL, CL, DL) + self->buf.w[RL[round][w]] + KL[round]) +
        EL;
    AL = EL;
    EL = DL;
    DL = ROL(10, CL);
  ...
  for (w = 0; w < 16; w++) { /* right line */
    T = ROL(SR[round][w],
            AR + F5(BR, CR, DR) + self->buf.w[RR[round][w]] + KR[round]) +
        ER;
    AR = ER;
    ER = DR;
    DR = ROL(10, CR);
  ...
  for (w = 0; w < 16; w++) { /* left line */
    T = ROL(SL[round][w],
            AL + F2(BL, CL, DL) + self->buf.w[RL[round][w]] + KL[round]) +
        EL;
    AL = EL;
    EL = DL;
    DL = ROL(10, CL);
  ...
  for (w = 0; w < 16; w++) { /* right line */
    T = ROL(SR[round][w],
            AR + F4(BR, CR, DR) + self->buf.w[RR[round][w]] + KR[round]) +
        ER;
    AR = ER;
    ER = DR;
    DR = ROL(10, CR);
  ...
  for (w = 0; w < 16; w++) { /* left line */
    T = ROL(SL[round][w],
            AL + F3(BL, CL, DL) + self->buf.w[RL[round][w]] + KL[round]) +
        EL;
    AL = EL;
    EL = DL;
    DL = ROL(10, CL);
  ...
  for (w = 0; w < 16; w++) { /* right line */
    T = ROL(SR[round][w],
            AR + F3(BR, CR, DR) + self->buf.w[RR[round][w]] + KR[round]) +
        ER;
    AR = ER;
    ER = DR;
    DR = ROL(10, CR);
  ...
  for (w = 0; w < 16; w++) { /* left line */
    T = ROL(SL[round][w],
            AL + F4(BL, CL, DL) + self->buf.w[RL[round][w]] + KL[round]) +
        EL;
    AL = EL;
    EL = DL;
    DL = ROL(10, CL);
  ...
  for (w = 0; w < 16; w++) { /* right line */
    T = ROL(SR[round][w],
            AR + F2(BR, CR, DR) + self->buf.w[RR[round][w]] + KR[round]) +
        ER;
    AR = ER;
    ER = DR;
    DR = ROL(10, CR);
  ...
  for (w = 0; w < 16; w++) { /* left line */
    T = ROL(SL[round][w],
            AL + F5(BL, CL, DL) + self->buf.w[RL[round][w]] + KL[round]) +
        EL;
    AL = EL;
    EL = DL;
    DL = ROL(10, CL);
  ...
  for (w = 0; w < 16; w++) { /* right line */
    T = ROL(SR[round][w],
            AR + F1(BR, CR, DR) + self->buf.w[RR[round][w]] + KR[round]) +
        ER;
    AR = ER;
    ER = DR;
    DR = ROL(10, CR);
...
void ripemd160_update(ripemd160_state* self, const unsigned char* p,
                      int length) {
