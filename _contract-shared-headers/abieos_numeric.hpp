...
// #define eosio::check(a, b) \
//   if ((!(a))) throw std::runtime_error(b);
// #endif
...
auto get_base58_map() {
  if (!map_initialized) {
    for (unsigned i = 0; i < base58_map.size(); ++i) base58_map[i] = -1;
...
std::array<uint8_t, size> decimal_to_binary(std::string_view s) {
  ...
  for (auto& src_digit : s) {
    if (src_digit < '0' || src_digit > '9') eosio::check(0, "invalid number");
    ...
    if (carry) eosio::check(0, "number is out of range");
  }
  return result;
}
...
std::array<uint8_t, size> base58_to_binary(std::string_view s) {
  ...
  for (auto& src_digit : s) {
    ...
    if (carry < 0) eosio::check(0, "invalid base-58 value");
    for (auto& result_byte : result) {
      int x = result_byte * 58 + carry;
      result_byte = x;
      carry = x >> 8;
    }
    if (carry) eosio::check(0, "base-58 value is out of range");
  }
  std::reverse(result.begin(), result.end());
  return result;
}
...
auto digest_message_ripemd160(const unsigned char* message,
                              size_t message_len) {
  std::array<unsigned char, 20> digest;
  ripemd160::ripemd160_state self;
  ripemd160::ripemd160_init(&self);
  ripemd160::ripemd160_update(&self, message, message_len);
  eosio::check(ripemd160::ripemd160_digest(&self, digest.data()),
               "ripemd failed");
  return digest;
}

template <size_t size, int suffix_size>
auto digest_suffix_ripemd160(const std::array<uint8_t, size>& data,
...
Key string_to_key(std::string_view s, key_type type,
  ...
  eosio::check(
      memcmp(ripe_digest.data(), whole.data() + result.data.size(), 4) == 0,
      "checksum doesn't match");
  return result;
}

template <typename Key, int suffix_size>
std::string key_to_string(const Key& key, const char (&suffix)[suffix_size],
                          const char* prefix) {
  static const auto size = std::tuple_size<decltype(Key::data)>::value;
...
public_key string_to_public_key(std::string_view s) {
  if (s.size() >= 3 && s.substr(0, 3) == "EOS") {
    ...
    static_assert(whole.size() == key.data.size() + 4,
                  "Error: whole.size() != key.data.size() + 4");
    memcpy(key.data.data(), whole.data(), key.data.size());
    auto ripe_digest =
        digest_message_ripemd160(key.data.data(), key.data.size());
    eosio::check(
        memcmp(ripe_digest.data(), whole.data() + key.data.size(), 4) == 0,
        "Key checksum doesn't match");
    return key;
    ...
    return string_to_key<public_key>("", key_type::r1, "R1");
  }
}
